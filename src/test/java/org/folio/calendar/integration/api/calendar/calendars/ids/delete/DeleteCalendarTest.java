package org.folio.calendar.integration.api.calendar.calendars.ids.delete;
...
class DeleteCalendarTest extends BaseCalendarApiTest {
  ...
  void testMissingDelete() {
    Response response = sendCalendarDeleteRequest(Arrays.asList(UUIDs.UUID_0, UUIDs.UUID_1));
  ...
  void testSingleExistingDelete() {
    ...
    sendCalendarDeleteRequest(Arrays.asList(createdId1))
      .then()
      .statusCode(is(HttpStatus.NO_CONTENT.value()));

    sendCalendarGetRequest(Arrays.asList(createdId1))
      .then()
  ...
  void testPartialDelete() {
    ...
    sendCalendarGetRequest(Arrays.asList(createdId2)).then().statusCode(is(HttpStatus.OK.value()));
  }
  ...
  void testMultipleDelete() {
    ...
    sendCalendarDeleteRequest(Arrays.asList(createdId1, createdId2))
      .then()
      .statusCode(is(HttpStatus.NO_CONTENT.value()));
    ...
    sendCalendarGetRequest(Arrays.asList(createdId1))
      .then()
      .statusCode(is(HttpStatus.NOT_FOUND.value()));
    sendCalendarGetRequest(Arrays.asList(createdId2))
      .then()
      .statusCode(is(HttpStatus.NOT_FOUND.value()));
  }
  ...
  void testDuplicateDelete() {
    ...
    sendCalendarDeleteRequest(Arrays.asList(createdId, createdId))
      .then()
      .statusCode(is(HttpStatus.NO_CONTENT.value()));

    sendCalendarGetRequest(Arrays.asList(createdId))
      .then()
      .statusCode(is(HttpStatus.NOT_FOUND.value()));
  }
  ...
  void testPartialMissing() {
    ...
    Response response = sendCalendarDeleteRequest(Arrays.asList(createdId, UUIDs.UUID_0));
    response.then().statusCode(is(HttpStatus.NOT_FOUND.value()));
