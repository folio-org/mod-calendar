...
class SearchCalendarTest extends BaseCalendarApiTest {
  ...
  void testIdSearch() {
    ...
    assertThat(
      resultAfter.getCalendars().stream().map(c -> c.withMetadata(null)).toList(),
  ...
  void testPagination() {
    ...
    assertThat(
      resultZeroLimit.getCalendars().stream().map(c -> c.withMetadata(null)).toList(),
      hasSize(0)
    );
    ...
    assertThat(
      resultFirstPage.getCalendars().stream().map(c -> c.withMetadata(null)).toList(),
    ...
    assertThat(
      resultSecondPage.getCalendars().stream().map(c -> c.withMetadata(null)).toList(),
    ...
    assertThat(
      resultLastPage.getCalendars().stream().map(c -> c.withMetadata(null)).toList(),
    ...
    assertThat(
      resultMiddleOfPage.getCalendars().stream().map(c -> c.withMetadata(null)).toList(),
    ...
    assertThat(
      resultOutOfBounds.getCalendars().stream().map(c -> c.withMetadata(null)).toList(),
      hasSize(0)
    );
  }
}
